...
inline const vector<string> builtin_names{"cd",  "echo", "exit",
                                                    "pwd", "type", "complete"};
...
inline void handle_exit(const vector<string> &) {
  exit(EXIT_SUCCESS);
}

inline void handle_pwd(const vector<string> &) {
...
inline void handle_type(const vector<string> &args) {
  ...
  } else if (string file_path = executable_in_path(target);
             !file_path.empty()) {
