...
inline bool is_file_executable(const string &path) {
...
inline string executable_in_path(const string &file_name) {
  ...
  for (const string &directory : directories_in_path()) {
    string full_path =
        (filesystem::path(directory) / file_name).string();
...
inline vector<string> executable_names_in_path() {
  ...
  return vector<string>(executable_names.begin(),
                                  executable_names.end());
}

inline vector<string>
entries_in_directory(const string &partial_path) {
  filesystem::path path(partial_path);
  filesystem::path directory = path.parent_path();
  unordered_set<string> entries;

  error_code error;
  for (const auto &directory_entry : filesystem::directory_iterator(
           directory.empty() ? "." : directory, error)) {
